...
public class DeviceConfiguration
{
    ...
    public static Dictionary<string, DeviceConfiguration?> GetDeviceConfigurations(string config)
    {
        return Directory.EnumerateFiles("Configuration", config, SearchOption.AllDirectories)
            .Select(f => new KeyValuePair<string, DeviceConfiguration?>(f.Split('/').Last(), JsonConvert.DeserializeObject<DeviceConfiguration>(File.ReadAllText(f))))
            .Where(kvp => kvp.Key != "ViLA.json")
            .ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
    }
